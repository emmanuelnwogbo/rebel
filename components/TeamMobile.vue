<template>
    <div class="teammobile">
        <h2 class="teammobile__header">Meet The Core Team</h2>
        <div class="teammobile__items">
            <div class="teammobile__item" @click='openLink("https://twitter.com/kvngspice_")'>
                <div class="teammobile__item--left">
                    <figure>
                        <img src="@/assets/imgs/samson.png"/>
                    </figure>
                </div>
                <div class="teammobile__item--right">
                    <h3>Samson</h3>
                    <p>founder/artist</p>
                    <span class="teammobile__item--social">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>                
                </div>
            </div>
            <div class="teammobile__item" @click='openLink("https://twitter.com/theimamadetona")'>
                <div class="teammobile__item--left">
                    <figure>
                        <img src="@/assets/imgs/imam.png"/>
                    </figure>
                </div>
                <div class="teammobile__item--right">
                    <h3>Imam</h3>
                    <p>project manager</p>
                    <span class="teammobile__item--social">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>                
                </div>
            </div>
            <div class="teammobile__item" @click='openLink("https://twitter.com/nerdyemmanuel")'>
                <div class="teammobile__item--left">
                    <figure>
                        <img src="@/assets/imgs/emmanuel.png"/>
                    </figure>
                </div>
                <div class="teammobile__item--right">
                    <h3>Emmanuel</h3>
                    <p>engineering</p>
                    <span class="teammobile__item--social">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>                
                </div>
            </div>
            <div class="teammobile__item" @click='openLink("https://twitter.com/octomint2000")'>
                <div class="teammobile__item--left">
                    <figure>
                        <img src="@/assets/imgs/kate.png"/>
                    </figure>
                </div>
                <div class="teammobile__item--right">
                    <h3>Kate</h3>
                    <p>community</p>
                    <span class="teammobile__item--social">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>                
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    export default {
        name: "TeamMobile",
        methods: {
            openLink(url) {
                window.open(url);
            }
        }        
    }
</script>

<style scoped lang="scss">
  @function scaleValue($value) {
    @return calc(
      #{$value} * (clamp(350px, 100vw, 3840px) / var(--ideal-viewport-width))
    );
  }

  .teammobile {
      padding-top: #{scaleValue(80)};
      margin-top: #{scaleValue(60)};

      &__header {
          color: #fff;
          text-align: left;
          font-size: #{scaleValue(90)};
          margin-bottom: #{scaleValue(130)};
      } 

      &__item {
          background: #C4C4C4;
          padding: #{scaleValue(30)};
          display: flex;
          border-radius: 1rem;
          flex-shrink: 0;
          margin-bottom: #{scaleValue(130)};

          &--left {
              margin-right: #{scaleValue(60)};

              & figure {
                  overflow: hidden;
                  border-radius: 1rem;
                  height: #{scaleValue(600)};
                  width: #{scaleValue(600)};

                  & img {
                      height: 100%;
                      width: 100%;
                      object-fit: cover;
                  }
              }
          }

          &--right {
              font-size: #{scaleValue(60)};
              height: 100%;
              padding-top: #{scaleValue(70)};
              text-transform: uppercase;

              & h3 {
                  font-size: #{scaleValue(75)};
                  margin-bottom: #{scaleValue(14)};
              }

              & p {
                  font-weight: 500;
              }
          }

          &--social {
              display: inline-block;
              margin-top: #{scaleValue(40)};
              cursor: pointer;

              & svg {
                  height: #{scaleValue(150)};
                  width: #{scaleValue(150)};
              }
          }
      }     
  }
  </style>