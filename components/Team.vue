<template>
    <div class="team" id="team">
        <h2 class="team__header">Meet the Core Team</h2>  
        <div class="team__items">
            <div class="team__item">
                <div class="team__item--left">
                    <div class="team__item--flipper">
                        <Flipper />
                    </div>
                    <figure class="team__item--fig">
                        <img src="@/assets/imgs/samson.png"/>
                    </figure>
                </div>
                <div class="team__item--right">
                    <h3>Samson</h3>
                    <p>founder/artist</p>
                    <span class="team__item--social" @click='openLink("https://twitter.com/kvngspice_")'>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>
                </div>
            </div>
             <div class="team__item">
                <div class="team__item--left">
                    <div class="team__item--flipper">
                        <Flipper />
                    </div>
                    <figure class="team__item--fig">
                        <img src="@/assets/imgs/imam.png"/>
                    </figure>
                </div>
                <div class="team__item--right">
                    <h3>Imam</h3>
                    <p>project manager</p>
                    <span class="team__item--social" @click='openLink("https://twitter.com/theimamadetona")'>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>
                </div>
            </div>
             <div class="team__item">
                <div class="team__item--left">
                    <div class="team__item--flipper">
                        <Flipper />
                    </div>  
                    <figure class="team__item--fig">
                        <img src="@/assets/imgs/emmanuel.png"/>
                    </figure>
                </div>
                <div class="team__item--right">
                    <h3>Emmanuel</h3>
                    <p>engineering</p>
                    <span class="team__item--social" @click='openLink("https://twitter.com/nerdyemmanuel")'>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>
                </div>
            </div>
             <div class="team__item">
                <div class="team__item--left">
                    <div class="team__item--flipper">
                        <Flipper />
                    </div>
                    <figure class="team__item--fig">
                        <img src="@/assets/imgs/kate.png"/>
                    </figure>
                </div>
                <div class="team__item--right">
                    <h3>Kate</h3>
                    <p>community</p>
                    <span class="team__item--social" @click='openLink("https://twitter.com/octomint2000")'>
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="45" viewBox="0 0 45 45" fill="none">
                            <rect width="45" height="45" fill="url(#pattern0)"/>
                            <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                            <use xlink:href="#image0_15_143" transform="scale(0.0104167)"/>
                            </pattern>
                            <image id="image0_15_143" width="96" height="96" xlink:href="data:image/png;base64,"/>
                            </defs>
                        </svg>
                    </span>
                </div>
            </div>
        </div>  
    </div>
</template>

<script>
    export default {
        name: "Team",
        methods: {
            openLink(url) {
                window.open(url);
            }
        }
    }
</script>

<style scoped lang="scss">
  @function scaleValue($value) {
    @return calc(
      #{$value} * (clamp(350px, 100vw, 3840px) / var(--ideal-viewport-width))
    );
  }

  .team {
     padding: #{scaleValue(75)} #{scaleValue(130)};
     padding-bottom: #{scaleValue(60)};
     padding-top: 0;

      &__header {
          color: #fff;
          text-align: center;
          font-size: #{scaleValue(40)};
          margin-bottom: #{scaleValue(50)};
      }

      &__items {
          display: flex;
          justify-content: space-between;
      }

      &__item {
          background: #C4C4C4;
          padding: #{scaleValue(10)};
          display: flex;
          align-items: center;
          width: #{scaleValue(320)};
          border-radius: 1rem;
          flex-shrink: 0;
          cursor: pointer;

          &:hover > &--left > &--flipper {
              opacity: 1;
          }

          &--left {
              margin-right: #{scaleValue(15)};
              position: relative;
          }

          &--flipper {
              position: absolute;
              top: 0;
              left: 0;
              width: 100%;
              height: 100%;
              z-index: 2;
              opacity: 0;
              transition: all .3s ease-in;
          }

          &--fig {
              height: #{scaleValue(152)};
              width: #{scaleValue(140)};
              border-radius: 1rem;
              overflow: hidden;

              & img {
                  height: 100%;
                  width: 100%;
                  object-fit: cover;
              }
          }

          &--right {
              height: 100%;
              padding-top: #{scaleValue(40)};
              text-transform: uppercase;

              & h3 {
                  font-size: #{scaleValue(17)};
                  margin-bottom: #{scaleValue(5)};
              }

              & p {
                  font-size: #{scaleValue(13)};
                  font-weight: 500;
              }
          }

          &--social {
              display: inline-block;
              margin-top: #{scaleValue(10)};
              cursor: pointer;
              transition: all .3s ease-in;

              &:hover {
                transform: translateY(#{scaleValue(-2)});
              } 

              & svg {
                  height: #{scaleValue(35)};
                  width: #{scaleValue(35)};
              }
          }
      }
  }
</style>